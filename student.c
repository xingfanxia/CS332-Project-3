...
/*
* enum is useful C language construct to associate desriptive words with integer
* values
* in this case the variable "alg" is created to be of the given enum type, which
* allows
...
static pcb_t* pop_high_priority() {
  ...
  if (head == NULL) {
    ...
    while (curr != NULL) {
      if (curr->static_priority == highest) {
        popped = curr;
        // if the porc we took is the head
        if (curr == head) {
          head = curr->next;
        }
        // if the porc we took is the tail
        if (curr == tail) {

          tail = prev;
        }
        // if the porc we took is in the middle
...
/*
* getReadyProcess removes a process from the front of a pseudo linked list (each
*process
* struct contains a pointer next that you can use to chain them together)
